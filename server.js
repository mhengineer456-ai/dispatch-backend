...
const handlePostRequest = async (req, res) => {
  ...
  try {
    ...
    if (action === 'createPackingList' || action === 'saveDraft' || p.status || p.documentType) {
      ...
      if (isDraft) {
        console.log('💾 Saving Draft Packing List to database...');
        ...
        console.log('💾 Saving FINAL Bill to database & starting background sync...');
        const savedBill = db.saveBill(billData);

        // Fast instant response to frontend (<50ms)
        res.json({
          success: true,
